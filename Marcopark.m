...
%%
    ...
    Nns_fuel(1,1)       = Nns_1(1,1)+Nns_2(1,1)+Nns_3(1,1);
    Nns_electric(1,1)   = Nns_1(1,2)+Nns_2(1,2)+Nns_3(1,2);
    Np(1,1)             = Np_2(1,1)+Np_3(1,1)+Np_2(1,2)+Np_3(1,2); %parking can be both garage and on-street parkers
    Np_fuel(1,1)        = Np_2(1,1)+Np_3(1,1);
    Np_electric(1,1)    = Np_2(1,2)+Np_3(1,2);
    Ns(1,1)             = Ns_2(1,1)+Ns_3(1,1)+Ns_2(1,2)+Ns_3(1,2); %searching can only be on-street parkers
    Ns_fuel(1,1)        = Ns_2(1,1)+Ns_3(1,1);
    Ns_electric(1,1)    = Ns_2(1,2)+Ns_3(1,2);
%%
...
%     Np_3(i+1,1)  = Np_3(i,1)+findparking_3(i,1)-departparking_3(i,1);% only on-street parked vehicles
    ...
    Nns_fuel(i+1,1)     = Nns_1(i+1,1)+Nns_2(i+1,1)+Nns_3(i+1,1);
    Nns_electric(i+1,1) = Nns_1(i+1,2)+Nns_2(i+1,2)+Nns_3(i+1,2);
    Np(i+1,1)           = Np_2(i+1,1)+Np_3(i+1,1)+Np_2(i+1,2)+Np_3(i+1,2);
    Np_fuel(i+1,1)      = Np_2(i+1,1)+Np_3(i+1,1);
    Np_electric(i+1,1)  = Np_2(i+1,2)+Np_3(i+1,2);
    Ns(i+1,1)           = Ns_2(i+1,1)+Ns_3(i+1,1)+Ns_2(i+1,2)+Ns_3(i+1,2);
    Ns_fuel(i+1,1)      = Ns_2(i+1,1)+Ns_3(i+1,1);
    Ns_electric(i+1,1)  = Ns_2(i+1,2)+Ns_3(i+1,2);
    
    density(i+1,1)=(Nns(i+1,1)+Ns(i+1,1))/Lnetwork;
    density_fuel(i+1,1)=(Nns_fuel(i+1,1)+Ns_fuel(i+1,1))/Lnetwork;
    density_electric(i+1,2)=(Nns_electric(i+1,1)+Ns_electric(i+1,1))/Lnetwork;
    if density(i+1,1)<=kc
        speed(i+1,1)=v;
    ...
    if density_fuel(i+1,1)<=kc
        speed_fuel(i+1,1)=v;
    elseif density_fuel(i+1,1)<=kj
        speed_fuel(i+1,1)=Qmax/(kc-kj)*(1-kj/density_fuel(i+1,1));
    else
        speed_fuel(i+1,1)=0;
    end
    
    if density_electric(i+1,1)<=kc
        speed_electric(i+1,1)=v;
    elseif density_electric(i+1,1)<=kj
        speed_electric(i+1,1)=Qmax/(kc-kj)*(1-kj/density_electric(i+1,1));
    else
        speed_electric(i+1,1)=0;
    end
...
% fuel vehicles (e = 1)
[average_searching_time_fuel, average_searching_distance_fuel, final_cum_revenue_fuel] = ...
    b_new_outputs_from_the_matrix(Nns_fuel(:,1), Ns_fuel(:,1), Np_fuel(:,1), Nns_1(:,1), Nns_3(:,1), speed_fuel(:,1), cum_enterthearea, cum_enterthearea_3(:,1), ...
    decideforparking_3(:,1), decideforparking_3(:,2), parking_pricing, parkingduration_expectation_fuel, parkingduration_expectation_electric);

average_searching_time_fuel
average_searching_distance_fuel
final_cum_revenue_fuel

% electric vehicles (e = 2)
[average_searching_time_electric, average_searching_distance_electric, final_cum_revenue_electric] = ...
    b_new_outputs_from_the_matrix(Nns_electric(:,1), Ns_electric(:,1), Np_electric(:,1), Nns_1(:,2), Nns_3(:,2), speed_electric(:,1), cum_enterthearea, cum_enterthearea_3(:,2), ...
    decideforparking_3(:,1), decideforparking_3(:,2), parking_pricing, parkingduration_expectation_fuel, parkingduration_expectation_electric);

average_searching_time_electric
average_searching_distance_electric
final_cum_revenue_electric
