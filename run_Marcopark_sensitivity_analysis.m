...
% 4a.) Elasticity (actual rate, contour plot, demand proportion):
% demand entering the area (k = 1) and flexible demand
figure
for r = -0.4:0.1:0.4
    i = int16(r/0.1) + 5;
    for p = 0:1:maximum_proportion_value
        ...
        if r == 0
            ...
            z_value(i,p + 1) = abs(((z_value_mean_1a(i,p + 1) - z_value_mean_1a(5,p + 1))/z_value_mean_1a(5,p + 1))/r);
        end
    end
end
z_value_mean = movmean(movmean(z_value(:,:),4,2),4,1);
...
colormap(jet)
cb = colorbar();
set(cb, 'Limits', [0 2], 'Ticks', [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2], 'TickLabels', {'0.2', '0.4', '0.6', '0.8', '1', '1.2', '1.4', '1.6', '1.8', '2'})
caxis([0 2])
grid

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% 4b.) Elasticity (actual rate, contour plot, supply proportion):
% Total number of existing parking spaces in the area (k = 2) and flexible supply
figure
clearvars x_value y_value z_value z_value_mean xq yq F xi yi z_value_interp
for r = -0.4:0.1:0.4
    ...
    for p = 0:1:maximum_proportion_value
        ...
        if r == 0
            z_value(i,p + 1) = 0;
        elseif z_value_mean_1b(5,p + 1) == 0
            z_value(i,p + 1) = 0;
        else
            z_value(i,p + 1) = abs(((z_value_mean_1b(i,p + 1) - z_value_mean_1b(5,p + 1))/z_value_mean_1b(5,p + 1))/r);
        end
    end
end
z_value_mean = movmean(movmean(z_value(:,:),4,2),4,1);
...
colormap(jet)
cb = colorbar();
set(cb, 'Limits', [0 8], 'Ticks', [0, 1, 2, 3, 4, 5, 6, 7, 8], 'TickLabels', {'0', '1', '2', '3', '4', '5', '6', '7', '8'})
caxis([0 8])
grid

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% 5a.) Elasticity (actual rate, contour plot, demand proportion):
% Average parking duration in the area (k = 3) and flexible demand
figure
clearvars x_value y_value z_value z_value_mean xq yq F xi yi z_value_interp
for r = -0.4:0.1:0.4
    ...
    for p = 0:1:maximum_proportion_value
        ...
        if r == 0
            ...
            z_value(i,p + 1) = abs(((z_value_mean_2a(i,p + 1) - z_value_mean_2a(5,p + 1))/z_value_mean_2a(5,p + 1))/r);
        end
    end
end
z_value_mean = movmean(movmean(z_value(:,:),4,2),4,1);
...
% 5b.) Elasticity (actual rate, contour plot, supply proportion):
% Total number of existing parking spaces in the area (k = 2) and flexible supply
figure
clearvars x_value y_value z_value z_value_mean xq yq F xi yi z_value_interp
for r = -0.4:0.1:0.4
    ...
    for p = 0:1:maximum_proportion_value
        ...
        if r == 0
            z_value(i,p + 1) = 0;
        elseif z_value_mean_2b(5,p + 1) == 0
            z_value(i,p + 1) = 0;    
        else
            z_value(i,p + 1) = abs(((z_value_mean_2b(i,p + 1) - z_value_mean_2b(5,p + 1))/z_value_mean_2b(5,p + 1))/r);
        end
    end
end
z_value_mean = movmean(movmean(z_value(:,:),4,2),4,1);
